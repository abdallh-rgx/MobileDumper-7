...
ParamNameCollisionCount: {};
	                   ...
	                   StringifyCollisionType(static_cast<ECollisionType>(OwnType)),
	                   MemberNameCollisionCount,
	                   SuperMemberNameCollisionCount,
	                   FunctionNameCollisionCount,
	                   SuperFuncNameCollisionCount,
...
uint64 CollisionManager::AddNameToContainer(NameContainer& StructNames, UEStruct Struct, std::pair<HashStringTableIndex, bool>&& NamePair, ECollisionType CurrentType, bool bIsStruct, UEFunction Func)
{
	...
	auto ResolveEffectiveNameConflicts = [&](NameContainer* TargetContainer) -> void
	{
		...
		const ECollisionType OwnType = static_cast<ECollisionType>(NewInfo.OwnType);

		if (OwnType != ECollisionType::MemberName && OwnType != ECollisionType::ParameterName)
			return;
		...
		while (bFoundConflict)
		{
			...
			for (size_t i = 0; i < NewInfoIndex; i++)
			{
				...
				{
					const uint32 CollisionCount = OwnType == ECollisionType::ParameterName ? NewInfo.ParamNameCollisionCount : NewInfo.MemberNameCollisionCount;

					if (CollisionCount >= ((1u << PerCountBitCount) - 1))
						return;

					if (OwnType == ECollisionType::ParameterName)
